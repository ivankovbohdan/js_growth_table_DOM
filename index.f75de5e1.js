...
appendRow.addEventListener("click", ()=>{
    ...
    tbody.append(newRow);
    if (table.rows.length >= 10) appendRow.setAttribute("disabled", true);
    else removeRow.removeAttribute("disabled");
});
removeRow.addEventListener("click", ()=>{
    const lastTr = table.rows[table.rows.length - 1];
    lastTr.remove();
    if (table.rows.length <= 2) removeRow.setAttribute("disabled", true);
...
appendColumn.addEventListener("click", ()=>{
    Array.from(tbody.rows).forEach((row)=>{
        const newCell = document.createElement("td");
        row.append(newCell);
        if (row.cells.length >= 10) appendColumn.setAttribute("disabled", true);
...
removeColumn.addEventListener("click", ()=>{
    Array.from(table.rows).forEach((row)=>{
        ...
        lastTd.remove();
        if (row.cells.length <= 2) removeColumn.setAttribute("disabled", true);
